  ...
  <View style={styles.patternContainer} pointerEvents="none">
    <View style={styles.patternGrid}>
      {Array.from({ length: 12 }).map((_, i) => (
        <View key={i} style={[styles.patternItem, { transform: [{ rotate: i % 2 === 0 ? '15deg' : '-15deg' }, { translateY: i % 2 === 0 ? 8 : 0 }] }]}>
...
export default function HomeScreen() {
  ...
  const [notes, setNotes] = useState<{ id: number, date: string, text: string }[]>([
    { id: 1, date: '2025-12-20', text: 'Luna yıllık aşılarını oldu, iştahı yerinde.' }
  ]);
  const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
  const [compatibilityInput, setCompatibilityInput] = useState({ p1: '', p2: '' });
    ...
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      >
        <View style={styles.modalOverlay}>
          <TouchableOpacity style={styles.modalBackdrop} onPress={() => setIsMenuOpen(false)} activeOpacity={1} />
          ...
          <View style={[styles.drawerContent, { paddingTop: insets.top }]}>
            ...
            <ScrollView style={styles.drawerMenu} contentContainerStyle={{ gap: 8 }}>
              {[
                ...
                <TouchableOpacity
                  key={item.id}
                  style={[styles.drawerItem, view === item.id && styles.drawerItemActive]}
                  onPress={() => navigateTo(item.id)}
                >
                  <item.icon size={20} color={view === item.id ? colors.stone800 : colors.stone400} strokeWidth={view === item.id ? 2.5 : 2} />
                  <Text style={[styles.drawerItemText, view === item.id && styles.drawerItemTextActive]}>{item.label}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
      ...
      <View style={styles.mainContent}>
        ...
        >
          ...
          {view === 'home' && (
            <View>
              <Text style={styles.pageTitle}>Teknik {'\n'}<Text style={{ color: colors.primary }}>Kontrol Paneli</Text></Text>
              ...
              >
                <View style={{ zIndex: 10 }}>
                  <View style={styles.systemBadge}><Text style={styles.systemBadgeText}>SISTEM ÇEVRIMIÇI</Text></View>
                  <Text style={styles.heroCardTitle}>Görsel Tarama</Text>
                  <Text style={styles.heroCardDesc}>Biyometrik veriler için tarayıcıyı aktif edin.</Text>
              ...
              <View style={styles.shortcutGrid}>
                <TouchableOpacity style={styles.shortcutCard} onPress={() => setView('compatibility')}>
                  <View style={[styles.shortcutIcon, { backgroundColor: colors.rose50 }]}>
                    <Users size={22} color={colors.rose400} />
                  </View>
                  <Text style={styles.shortcutTitle}>Uyum Analizi</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.shortcutCard} onPress={() => setView('notes')}>
                  <View style={[styles.shortcutIcon, { backgroundColor: colors.stone50 }]}>
          ...
          {view === 'pets' && (
            <View>
              <View style={styles.sectionHeader}>
                ...
                <TouchableOpacity style={styles.addBtn}><Plus size={20} color="#fff" /></TouchableOpacity>
              </View>
              {pets.map(pet => (
                <View key={pet.id} style={styles.petCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={styles.petTags}>
                      <View style={styles.tag}><Text style={styles.tagText}>{pet.age} Yaş</Text></View>
                      <View style={[styles.tag, { backgroundColor: colors.green50 }]}>
                        <CheckCircle2 size={10} color={colors.green600} />
                        <Text style={[styles.tagText, { color: colors.green600, marginLeft: 4 }]}>Sağlıklı</Text>
                      </View>
                    </View>
                  </View>
                  <TouchableOpacity><Settings size={20} color={colors.stone300} /></TouchableOpacity>
                </View>
              ))}
            </View>
          )}
          ...
          {view === 'notes' && (
            <View>
              ...
              <Text style={styles.sectionDesc}>Veteriner ziyareti öncesi tarihsel gelişim verileri.</Text>

              <View style={styles.datePickerCard}>
                <View style={styles.dateIconBox}><Calendar size={20} color={colors.stone400} /></View>
                <View style={{ flex: 1 }}>
              ...
              {notes.filter(n => n.date === selectedDate).length === 0 && (
                <View style={styles.emptyState}>
                  <Text style={styles.emptyText}>Bu tarih için kayıt bulunamadı.</Text>
                </View>
              )}
              ...
              {notes.filter(n => n.date === selectedDate).map(note => (
                <View key={note.id} style={styles.noteItem}>
                  ...
                  <View style={styles.noteFooter}>
                    <View style={styles.dateBadge}><Text style={styles.dateBadgeText}>{note.date}</Text></View>
          ...
          {view === 'compatibility' && (
            <View>
              ...
              <Text style={styles.sectionDesc}>İki farklı türün veya cinsin bir arada yaşama potansiyeli.</Text>
              ...
              <View style={styles.compContainer}>
                <View style={styles.compInputCard}>
                  ...
                  <View style={styles.compInputRow}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(t) => setCompatibilityInput({ ...compatibilityInput, p1: t })}
                    />
                  </View>
                </View>
                ...
                <View style={styles.heartConnector}>
                  <View style={styles.heartCircle}>
                    <Heart size={24} color={colors.rose400} fill={colors.rose400} />
                  </View>
                </View>
                ...
                <View style={styles.compInputCard}>
                  ...
                  <View style={styles.compInputRow}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(t) => setCompatibilityInput({ ...compatibilityInput, p2: t })}
                    />
                  </View>
                </View>
              </View>

              <TouchableOpacity style={styles.analyzeBtn} onPress={handleCompatibilityCheck}>
          ...
          {isAnalyzing && (
            <View style={styles.loaderView}>
              ...
              <Text style={styles.loaderDesc}>Gemini AI biyometrik katmanları tarıyor...</Text>
            </View>
          )}
          ...
          {view === 'result' && aiResponse && (
            <View>
              ...
              <TouchableOpacity style={styles.analyzeBtn} onPress={() => setView('home')}>
          ...
          {view === 'settings' && (
            <View>
              ...
              <View style={[styles.compInputCard, { marginBottom: 20 }]}>
                ...
                <TextInput style={styles.settingsInput} defaultValue="Pati Sever" />
              </View>
              <View style={[styles.compInputCard, { marginBottom: 20 }]}>
                <Text style={styles.inputLabel}>E-POSTA PROTOKOLÜ</Text>
                <TextInput style={styles.settingsInput} defaultValue="kullanici@pawmate.com" />
              </View>
      ...
      < View style={[styles.bottomNav, { paddingBottom: insets.bottom + 10 }]} >
        ...
        <TouchableOpacity style={styles.navItem} onPress={() => setView('notes')}>
